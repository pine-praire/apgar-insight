-- user_roles: sparse table — a row here means the user is an admin
...
-- security-definer function: joins auth.users (normally RLS-restricted) with profiles
-- auth.uid() resolves from the calling session JWT even inside a security-definer context
...
as $$
begin
  -- in-body admin check: defence-in-depth even if called outside PostgREST
  if not exists (
    select 1 from public.user_roles
    where user_id = auth.uid() and role = 'admin'
  ) then
...
-- lock down execute: only authenticated users may call this function
